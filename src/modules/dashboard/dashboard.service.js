...
const dashboardService = {
  async summary() {
    ...
    const [newEnquiries, pipeline, byType, needsVerification, availability, activeSelections, recent] =
      await Promise.all([
    ...
    const currentEdit = (await editRepository.findLive()).find((e) => e.status === "current") ?? null;
  ...
  async verificationQueue(limit = 25) {
    ...
    const { items: stale } = await stoneRepository.findMany({ limit: 200, page: 1, sort: "newest" });
    const overdue = stale
      .filter(
        (s) =>
